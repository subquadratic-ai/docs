...
SubQ doesn't insert line breaks at fixed intervals or arbitrary word counts. Instead, it analyzes different signals from the audio, such as:

- **Pause duration:** Extended silences between utterances often indicate a transition. In most cases, a speaker finalizing one thought and beginning another. SubQ's STT models use these pauses as signals for potential paragraph boundaries.

- **Change in topic and context:** SubQ's models evaluate whether the content that comes before and after a break represents a shift in the subject. For example, a speaker who pauses briefly to breathe mid-sentence won't trigger a paragraph break, but a speaker who finishes discussing Q1 revenue and moves to hiring plans will.
...
```bash
curl -X POST "https://api.subquadratic.ai/v1/listen?paragraphs=true" \
...
```bash
curl -X POST "https://api.subquadratic.ai/v1/listen?paragraphs=true&smart_format=true&numerals=true" \
  -H "Authorization: Bearer YOUR_SUBQ_API_KEY" \
  --data-binary @audio.wav
```
...
- **Meeting and call transcripts:** It makes it easier to scan through long-form recordings as reviewers can jump to specific sections instead of reading the entire transcript.
