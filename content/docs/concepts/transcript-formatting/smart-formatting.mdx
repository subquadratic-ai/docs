...
```bash
curl -X POST "https://stt-api.subq.ai/v1/listen?smart_format=true" \
...
```bash
curl -X POST "https://stt-api.subq.ai/v1/listen?numerals=true" \
  -H "Authorization: Bearer YOUR_SUBQ_API_KEY" \
  --data-binary @audio.wav
```
...
```bash
curl -X POST "https://stt-api.subq.ai/v1/listen?measurements=true" \
  -H "Authorization: Bearer YOUR_SUBQ_API_KEY" \
  --data-binary @audio.wav
```
