...
The Python Deepgram SDK uses a `DeepgramClientEnvironment` object to configure where API requests are sent. By default, it points at `api.deepgram.com`. To migrate, you create a custom environment that redirects all REST and WebSocket traffic to `api.aldea.ai` instead.
...
# Redirect all SDK traffic to the Aldea API
config = DeepgramClientEnvironment(
    base="https://api.aldea.ai",        # REST endpoint for pre-recorded transcription
    production="wss://api.aldea.ai",     # WebSocket endpoint for streaming
    agent="wss://api.aldea.ai"           # Agent WebSocket (set to same as production)
)
...
The Node.js SDK accepts a `global.url` option that overrides the base URL for all requests. Setting this to `https://api.aldea.ai` redirects both REST and WebSocket traffic.
...
The Go SDK accepts `Host` and `AccessToken` fields in the `ClientOptions` struct. Setting `Host` to `https://api.aldea.ai` redirects all API calls. Pass your Aldea key via `AccessToken` instead of the first positional argument.
...
client := dglisten.NewREST("", &dgi.ClientOptions{
    Host:        "https://api.aldea.ai",   // REST and WebSocket base URL
    AccessToken: "YOUR_ALDEA_API_KEY",      // Aldea API key (starts with org_)
})
```
...
Send a local audio file through the SDK to verify that REST transcription works with the Aldea backend.

<Tabs items={['Python', 'Node.js', 'Go']} groupId="language" persist updateAnchor>
<Tab value="Python" id="python">

This reads `audio.wav` from disk, sends it to Aldea via the SDK's `transcribe_file()` method, and prints the top transcript result:
...
This creates a read stream from `audio.wav`, sends it to Aldea for transcription, and logs the result. The `response?.result ?? response` pattern handles different SDK response wrapper versions:
...
This sends `audio.wav` to Aldea using the SDK's `DoFile()` method. The response is unmarshalled into a `Response` struct, and the transcript is extracted from the first channel's top alternative:
...
Streaming verification confirms that WebSocket connections work with the Aldea backend. This example connects to a live audio stream and prints transcripts as they arrive.

<Tabs items={['Node.js']}>
<Tab value="Node.js" id="node">

This opens a WebSocket connection to `wss://api.aldea.ai/v1/listen` through the SDK, listens for transcript events, and streams audio from a live internet radio source. The `LiveTranscriptionEvents.Transcript` event fires each time the server sends a transcript message, and the `LiveTranscriptionEvents.Open` event fires once the connection is established:
...
If you prefer not to use a Deepgram SDK, you can call the Aldea API directly using standard HTTP clients. The endpoint (`POST /v1/listen`) and JSON response format are identical whether you use an SDK or not. This approach gives you full control over the request and removes the Deepgram SDK dependency entirely.

<Tabs items={['Python (httpx)', 'Node.js (fetch)', 'Go (net/http)']}>
<Tab value="Python (httpx)">

Send audio bytes directly to the Aldea REST endpoint using `httpx`. The `Bearer` token authenticates the request, and the transcript is extracted from the JSON response:
...
Use the built-in `fetch` API to POST audio bytes to Aldea. The response JSON follows the same structure as the SDK response:
...
- [API reference](/docs/api-reference/api-overview)
