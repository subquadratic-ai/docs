...
```bash
curl -X POST "https://stt-api.subq.ai/v1/listen?paragraphs=true" \
...
```bash
curl -X POST "https://stt-api.subq.ai/v1/listen?paragraphs=true&smart_format=true&numerals=true" \
  -H "Authorization: Bearer YOUR_SUBQ_API_KEY" \
  --data-binary @audio.wav
```
