...
description: Transcribe audio from a URL using the SubQ API
---
...
2. **POST to `/v1/listen`:** You send the downloaded audio bytes to the SubQ API in the request body. This is the same endpoint and format used in [Transcribe a file](/docs/integration/transcribe-file). The only difference is where the audio comes from.
...
- [Migrate from Deepgram](/docs/integration/migrate-from-deepgram) - already using the Deepgram SDK? Switch to SubQ with a 2-line change
