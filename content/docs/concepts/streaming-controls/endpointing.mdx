...
wss://api.subquadratic.ai/v1/listen?endpointing=300&encoding=mp3

# Slower finalization (800ms), better for speakers who pause mid-sentence
wss://api.subquadratic.ai/v1/listen?endpointing=800&encoding=mp3
```

To disable automatic finalization, set `endpointing` to `false` as follows:

```bash
wss://api.subquadratic.ai/v1/listen?endpointing=false&encoding=mp3
```
