...
In this example, you capture microphone audio in the browser, stream it to the Aldea API over a WebSocket, and display the transcript in real time on the page. This builds on the same setup used in the [pre-recorded example](/docs/integration/browser/pre-recorded-upload) but introduces WebSocket streaming and microphone access.
...
The page loads the Deepgram SDK from a CDN, creates a client pointed at the Aldea API, and opens a persistent WebSocket connection:
...
`client.listen.live()` opens a WebSocket to `wss://api.aldea.ai/v1/listen` and handles authentication automatically using the API key.
...
Each chunk is sent directly to the Aldea API over the WebSocket with `connection.send()`.
...
<html>
  <head>
    <title>Aldea Live Transcription</title>
  </head>
